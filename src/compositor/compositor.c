                         ...
                         guint32      timestamp)
{
  ...
  meta_display_set_input_focus_xwindow (display,
                                        window,
                                        timestamp);
}
...
{
  if (!meta_is_wayland_compositor () &&
      event->type == meta_x11_display_get_damage_event_base (compositor->display->x11_display) + XDamageNotify)
    {
      ...
      if (window == NULL)
        {
          ...
          window = meta_x11_display_lookup_x_window (compositor->display->x11_display,
                                                     xwin);
        }
