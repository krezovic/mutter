      ...
      if (META_STACK_ID_IS_X11 (window))
        {
          MetaWindow *meta_window =
            meta_x11_display_lookup_x_window (tracker->display->x11_display,
                                              (Window)window);
  ...
  if (META_STACK_ID_IS_X11 (window))
    {
      ...
      if (changes.sibling != find_x11_sibling_upwards (tracker, window))
        {
          serial = XNextRequest (tracker->display->x11_display->xdisplay);

          meta_error_trap_push ();

          changes.stack_mode = changes.sibling ? Below : Above;

          XConfigureWindow (tracker->display->x11_display->xdisplay,
                            window,
