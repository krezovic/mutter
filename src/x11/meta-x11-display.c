...
static void on_cursor_updated   (MetaDisplay    *display,
...
meta_x11_display_dispose (GObject *object)
{
  ...
  if (g_signal_handler_is_connected (x11_display->display->bell,
    ...
    g_signal_handler_disconnect (x11_display->display->bell,
                                 x11_display->is_audible_changed_handler);
  ...
  if (g_signal_handler_is_connected (x11_display->display,
                                     x11_display->monitors_changed_handler))
    g_signal_handler_disconnect (x11_display->display,
                                 x11_display->monitors_changed_handler);
  ...
  if (g_signal_handler_is_connected (x11_display->display,
                                     x11_display->cursor_updated_handler))
    g_signal_handler_disconnect (x11_display->display,
                                 x11_display->cursor_updated_handler);
  ...
  if (x11_display->gdisplay)
    {
      gdk_display_close (x11_display->gdisplay);
      x11_display->gdisplay = NULL;
    }
 ...
 */
static void
x11_bell_init (MetaX11Display *x11_display)
{
...
x11_bell_shutdown (MetaX11Display *x11_display)
{
 ...
 */
static void
x11_bell_set_audible (MetaX11Display *x11_display,
                      gboolean is_audible)
{
...
on_is_audible_changed (MetaBell       *bell,
                       ...
                       MetaX11Display *x11_display)
{
  x11_bell_set_audible (x11_display, is_audible);
}
...
meta_x11_display_new (MetaDisplay *display, GError **error)
{
  ...
  gdisplay = gdk_display_open (xdisplay_name);
  ...
  /* We need to be able to fully trust that the window and monitor sizes
     ...
     scale handling */
  gdk_x11_display_set_window_scale (gdisplay, 1);
...
static void
on_cursor_updated (MetaDisplay    *display,
                   MetaX11Display *x11_display)
{
  update_cursor_theme (x11_display);
}
